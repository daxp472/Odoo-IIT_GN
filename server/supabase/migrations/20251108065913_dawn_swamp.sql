...
-- Contains DDL statements and sample data for the OneFlow backend

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- User profiles table - extends Supabase auth.users
-- Links to Supabase authentication and stores additional user information
...
-- Add the role_requests table after the profiles table and before the projects table
...
-- Projects table - main project management entity
-- Stores project information including budget, timeline, and financial tracking
...
-- Individual tasks within projects with assignment and tracking capabilities
CREATE TABLE tasks (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    title text NOT NULL,
    description text,
...
-- Timesheets table - time tracking for projects and tasks
-- Records billable and non-billable hours worked by team members
...
-- Sales Orders table - customer orders and quotes
-- Main sales order header information
...
-- Sales Order Lines table - detailed line items for sales orders
-- Individual items/services within a sales order
...
-- Purchases table - vendor purchases and procurement
...
-- Purchase Lines table - detailed line items for purchases
-- Individual items purchased from vendors
...
-- Expenses table - project and general expenses
-- Expense tracking for reimbursements and project costs
...
-- Invoices table - customer billing and invoicing
-- Invoice header information for billing clients
...
-- Invoice Lines table - detailed line items for invoices
-- Individual billable items on customer invoices
...
CREATE INDEX idx_timesheets_approved ON timesheets(approved);
...
-- Project Financials View - aggregated financial data per project
-- Provides quick access to revenue, costs, and profit calculations
...
SELECT 
    ...
    p.budget,
    p.status,
    COALESCE(invoice_revenue.total_revenue, 0) as total_revenue,
    COALESCE(expense_costs.total_expenses, 0) + COALESCE(purchase_costs.total_purchases, 0) as total_costs,
    COALESCE(invoice_revenue.total_revenue, 0) - (COALESCE(expense_costs.total_expenses, 0) + COALESCE(purchase_costs.total_purchases, 0)) as total_profit,
    COALESCE(timesheet_hours.total_hours, 0) as total_hours_logged,
...
LEFT JOIN (
    ...
    WHERE status = 'paid'
    GROUP BY project_id
) invoice_revenue ON p.id = invoice_revenue.project_id
LEFT JOIN (
    SELECT 
        project_id,
    ...
    WHERE approved = true
    GROUP BY project_id
) expense_costs ON p.id = expense_costs.project_id
LEFT JOIN (
    SELECT 
        project_id,
    ...
    GROUP BY project_id
) purchase_costs ON p.id = purchase_costs.project_id
LEFT JOIN (
    SELECT 
        project_id,
    ...
    FROM timesheets
    GROUP BY project_id
) timesheet_hours ON p.id = timesheet_hours.project_id
LEFT JOIN (
    SELECT 
        project_id,
...
-- Sample user profiles (these would typically be created via Supabase Auth)
